...
const playMusic = (track, pause = false) => {
    ...
    if (pause != true) {
        currentSong.play();
        play.src = "public/image/pause.svg"
    }
    ...
    try {
        ...
        for (let i = 0; i < anchor.length; i++) {
            ...
            if (e.href.includes("public/Songs/") && !e.href.includes(".htaccess")) {
                ...
                let a = await fetch(`public/Songs/${Folder}/zinfo.json`);
                let response = await a.json();
                cardcontainer.innerHTML = cardcontainer.innerHTML + `
                    <div data-folder="${Folder}" class="card">
                        <div class="play">
                            <img src="public/image/play-button.svg" alt="">
                        </div>
        ...
        songUl.innerHTML = songUl.innerHTML + `<li> 
            <div class="musicbox">
                <div class="songicon invert">
                    <div>
                        <img src="public/image/music.svg" alt="">
                    </div>
                </div>
                <div class="songinfo">
                ...
                <div class="playnow ">
                    <span>play now</span>
                    <span><img src="public/image/play.svg" class="invert1" alt=""></span>
                </div>
            </div>
        </li>`;
    }
...
function attachAlbumSelectEvent() {
    Array.from(document.getElementsByClassName("card")).forEach(e => {
        e.addEventListener("click", async item => {
            ...
            populateSongList();
            attachSongListEvent();
            document.getElementById("play").src = "public/image/play.svg";
...
function attachPlaybackControlEvents() {
    play.addEventListener("click", () => {
        if (currentSong.paused) {
            ...
            currentSong.pause();
            play.src = "public/image/play.svg"
        }
    })
    ...
    document.querySelector(".volume").getElementsByTagName("input")[0].addEventListener("change", (e) => {
        if (e.target.value / 100 == 0) {
            document.querySelector(".volume").getElementsByTagName("img").vo.src = "public/image/volume-off.svg"
            currentSong.volume = 0;
        } else {
            currentSong.volume = (e.target.value / 100);
            document.querySelector(".volume").getElementsByTagName("img").vo.src = "public/image/volume-on.svg"
        }
    })

    document.querySelector(".volume").getElementsByTagName("img")[0].addEventListener("click", (e) => {
        if (document.querySelector(".volume").getElementsByTagName("img").vo.src.split("public/image/")[1] == "volume-on.svg") {
            currentSong.volume = 0;
            document.querySelector(".volume").getElementsByTagName("img").vo.src = "public/image/volume-off.svg"
...
function updatePlayIcons(currentTrack) {
    ...
    songListItems.forEach(item => {
        ...
        if (item.querySelector(".songinfo div").textContent.trim() === decodeURI(currentTrack)) {
            playIcon.src = "public/image/pause.svg";
        } else {
            playIcon.src = "public/image/play.svg";
        }
    });
}
